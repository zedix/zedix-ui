...
- **Modal/inert functionality**. Opening the dialog as _modal_ will make the content behind <q>[inert](https://html.spec.whatwg.org/multipage/interaction.html#modal-dialogs-and-inert-subtrees)</q>.
...
- **CSS ::backdrop**. The [::backdrop pseudo-element](https://developer.chrome.com/blog/css-backdrop-inheritance), rendered immediately beneath the dialog, could be used to blur, darken, or obfuscate the inert content behind.

## Example

<Source
  dark
  code={`<zx-dialog title="Title">
  #shadow-root (open)
  ...
  <menu slot="actions">
    <zx-button variant="outline" invokeaction="close">Cancel<zx-button>
